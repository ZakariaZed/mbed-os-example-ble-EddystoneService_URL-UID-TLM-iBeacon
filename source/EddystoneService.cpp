    ...
    eventQueue(evQ)
{
    ...
    iBeaconPeriod = correctAdvertisementPeriod(paramsIn.iBeaconPeriod);
...
EddystoneService::EddystoneService(BLE                 &bleIn,
    ...
    ibeaconframe(),
    iBeaconPeriod(DEFAULT_IBEACON_PERIOD_MSEC),
    rawiBeacon(NULL),
    rawUrlFrame(NULL),
    rawUidFrame(NULL),
    rawTlmFrame(NULL),
...
void EddystoneService::setiBeacon(uint8_t uuid[16],
            uint16_t        majNum,
            uint16_t        minNum,
            uint8_t         txP,
            uint16_t        compID)
{
   ibeaconframe.setiBeacon(uuid, majNum, minNum, txP ,compID);
    rawiBeacon = ibeaconframe.data.rawFrame;
    // printf("rawiBeacon setBeacon =%d\n", rawiBeacon[6]);
}
/****************************************************************************************/
...
EddystoneService::EddystoneError_t EddystoneService::startConfigServiceafterdeconnexion()
{
    setupConfigService();
}
...
EddystoneService::EddystoneError_t EddystoneService::startBeaconService(void)
{
    if (operationMode == EDDYSTONE_MODE_BEACON) {
        ...
        return EDDYSTONE_ERROR_NONE;
    } else if (!urlFramePeriod && !uidFramePeriod && !tlmFramePeriod && !iBeaconPeriod) {
...
void EddystoneService::getEddystoneParams(EddystoneParams_t &params)
{
    ...
    params.urlDataLength  = urlFrame.getEncodedURLDataLength();
        /***/
        params.iBeaconPeriod = iBeaconPeriod;
        params.majorNumber = ibeaconframe.GetmajorNumber();
        params.minorNumber = ibeaconframe.GetminorNumber();
        params.companyID = ibeaconframe.GetcompanyID();
        params.txPower = ibeaconframe.GettxPower(); 
    /***/
    ...
    memcpy(params.uidInstanceID,  uidFrame.getUIDInstanceID(),  sizeof(UIDInstanceID_t));
       /***************************************************************************/
        memcpy(params.uuid,  ibeaconframe.GetrawFrame(),  sizeof(proximityUUID));
}
...
void EddystoneService::swapAdvertisedFrame(FrameType frameType)
{
    ...
    case EDDYSTONE_FRAME_UID:
        ...
        break;
     case EDDYSTONE_FRAME_IBEACON :

            ibeaconframe.constructiBeacon(rawiBeacon);
          //  printf("rawiBeacon  =%d\n", rawiBeacon[6]);
         //  updateAdvertisementPacket(rawiBeacon, ibeaconframe.getRawFrameSize());
           //printf(" rawiBeacon.getRawFrameSizeAAAAAAAAAA =%d\n",  rawiBeacon[0]);
           ble.gap().clearAdvertisingPayload();
           ble.gap().accumulateAdvertisingPayload(GapAdvertisingData::BREDR_NOT_SUPPORTED | GapAdvertisingData::LE_GENERAL_DISCOVERABLE );
           ble.gap().accumulateAdvertisingPayload(GapAdvertisingData::MANUFACTURER_SPECIFIC_DATA, ibeaconframe.data.rawFrame, ibeaconframe.getRawFrameSize());
...
void EddystoneService::setupBeaconService(void)
{
    ...
    if (tlmFramePeriod) {
        ...
        /*******************************************************************************/
            if (iBeaconPeriod) {
                rawiBeacon = new uint8_t[ibeaconframe.getRawFrameSize()];
                    ibeaconframe.constructiBeacon(rawiBeacon);
                 //    printf("rawiBeacon 2 =%d\n", rawiBeacon[6]);
                   // ibeaconframe.constructiBeacon(rawiBeacon);
                   // printf("iBeaconPeriod =%d\n", rawiBeacon[0]);
        /* Do not initialise because we have to reconstruct every 0.1 secs */
    }
    ...
    if (urlFramePeriod) {
        ...
        /************************************************************************************/
        if (iBeaconPeriod) {
        advFrameQueue.push(EDDYSTONE_FRAME_IBEACON);
        iBeaconCallbackHandle = eventQueue.call_every(
            iBeaconPeriod,
            Callback<void(FrameType)>(this, &EddystoneService::enqueueFrame),
            EDDYSTONE_FRAME_IBEACON /***/
        );
    }
...
void EddystoneService::manageRadio(void)
{
    ...
    if (advFrameQueue.pop(frameType)) {
       ...
       //apAdvertisedFrame(EDDYSTONE_FRAME_IBEACON);
        ble.gap().startAdvertising();
...
void EddystoneService::setupConfigService(void)
{
    ...
    /**/urlDataChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);    
    unlockChar->setWriteAuthorizationCallback(this, &EddystoneService::unlockAuthorizationCallback);
    /**/flagsChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);    
    urlDataChar->setWriteAuthorizationCallback(this, &EddystoneService::urlDataWriteAuthorizationCallback);
    /**/advPowerLevelsChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);     
    flagsChar->setWriteAuthorizationCallback(this, &EddystoneService::basicAuthorizationCallback<uint8_t>);
    /**/txPowerModeChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);   
    advPowerLevelsChar->setWriteAuthorizationCallback(this, &EddystoneService::basicAuthorizationCallback<PowerLevels_t>);
    /**/beaconPeriodChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);   
    txPowerModeChar->setWriteAuthorizationCallback(this, &EddystoneService::powerModeAuthorizationCallback);
    /**/resetChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);
    ...
    setupEddystoneConfigAdvertisements();
            /*
 * Ajouter un service TLM contenant la période et la température.
 */
        beaconPeriodTLMChar   = new ReadWriteGattCharacteristic<uint16_t>(UUID_BEACON_PERIOD_CHAR, &tlmFramePeriod);
        beaconPeriodTLMChar->setWriteAuthorizationCallback(this, &EddystoneService::basicAuthorizationCallback<uint16_t>);
       /***/ beaconPeriodTLMChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);      
        charTab[0] = beaconPeriodTLMChar;
        tlmtemperature= 0;
        TEMPERATURE   = new ReadOnlyGattCharacteristic<uint16_t>(UUID_TLMBEACON_PERIOD_CHAR, &tlmtemperature);      
        /***/TEMPERATURE->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);
        charTab[1] = TEMPERATURE;
        GattService configServiceTLM(UUID_TLM_BEACON_SERVICE, charTab, sizeof(charTab) / sizeof(GattCharacteristic *));
        ble.gattServer().addService(configServiceTLM);
        ble.gattServer().onDataWritten(this, &EddystoneService::onDataWrittenCallback);
        updatetemperatureValues(tlmtemperature);
        /*
 * Ajout d'un service UID contenant la période.
 */
        beaconPeriodUIDChar   = new ReadWriteGattCharacteristic<uint16_t>(UUID_BEACON_PERIOD_CHAR, &uidFramePeriod);
        beaconPeriodUIDChar->setWriteAuthorizationCallback(this, &EddystoneService::basicAuthorizationCallback<uint16_t>);
        /***/ beaconPeriodUIDChar->setReadAuthorizationCallback(this, &EddystoneService::basicAuthorCallback);      
        charT[0]= beaconPeriodUIDChar;
        GattService configServiceUID(UUID_UID_BEACON_SERVICE, charT, sizeof(charT) / sizeof(GattCharacteristic *));
        ble.gattServer().addService(configServiceUID);
        updateCharacteristicValues2();
}
void EddystoneService::basicAuthorCallback(GattReadAuthCallbackParams *authParams)
{
    if (Authentification ==12){
...
void EddystoneService::stopBeaconService(void)
{
    ...
    /****************************************************************************/
        if (iBeaconCallbackHandle) {
        eventQueue.cancel(iBeaconCallbackHandle);
        iBeaconCallbackHandle = 0;
                 }
}
...
/*
 * Fonction du update de température.
 */
void EddystoneService::updatetemperatureValues(uint8_t newLevel)
{   
    temperaturelevel = newLevel;
    tlmtemperature++;
  ble.gattServer().write(TEMPERATURE->getValueHandle(),reinterpret_cast<uint8_t *>(&tlmtemperature), sizeof(uint16_t));
    ble.gattServer().write(beaconPeriodTLMChar->getValueHandle(), reinterpret_cast<uint8_t *>(&tlmFramePeriod), sizeof(uint16_t));
}
void EddystoneService::updateCharacteristicValues2(void)
{
    ble.gattServer().write(beaconPeriodUIDChar->getValueHandle(), reinterpret_cast<uint8_t *>(&uidFramePeriod), sizeof(uint16_t));
    
}
void EddystoneService::authentificationcondition( uint8_t T)
{
    Authentification = T;
...
void EddystoneService::lockAuthorizationCallback(GattWriteAuthCallbackParams *authParams)
{
            if (Authentification ==12)
                ...
                {
                authParams->authorizationReply =AUTH_CALLBACK_REPLY_ATTERR_ATTRIBUTE_NOT_LONG;
                }
}
...
void EddystoneService::onDataWrittenCallback(const GattWriteCallbackParams *writeParams)
{
    ...
    if (handle == lockChar->getValueHandle()) {
        ...
        ble.gattServer().write(beaconPeriodChar->getValueHandle(), reinterpret_cast<uint8_t *>(&urlFramePeriod), sizeof(uint16_t));
        ble.gattServer().write(lockChar->getValueHandle(), lock, sizeof(PowerLevels_t));
    }else if (handle == beaconPeriodTLMChar->getValueHandle()) {
             uint16_t tmpBeaconPeriod = correctAdvertisementPeriod(*((uint16_t *)(writeParams->data)));
        if (tmpBeaconPeriod != tlmFramePeriod) {
            tlmFramePeriod = tmpBeaconPeriod;
            ble.gattServer().write(beaconPeriodTLMChar->getValueHandle(), reinterpret_cast<uint8_t *>(&tlmFramePeriod), sizeof(uint16_t));
        }
                
        }else if (handle == beaconPeriodUIDChar->getValueHandle()) {
                     uint16_t tmpBeaconPeriod = correctAdvertisementPeriod(*((uint16_t *)(writeParams->data)));
        if (tmpBeaconPeriod != uidFramePeriod) {
            uidFramePeriod = tmpBeaconPeriod;
            ble.gattServer().write(beaconPeriodUIDChar->getValueHandle(), reinterpret_cast<uint8_t *>(&uidFramePeriod), sizeof(uint16_t));
        }
...
/************************************************************************************************/
/***************************************************************************************************/
void EddystoneService::setiBeaconAdvertisingInterval(uint16_t iBeaconIntervalIn)
{
    if (iBeaconIntervalIn == iBeaconPeriod) {
        /* Do nothing */
        return;
    }

    /* Make sure the input period is within bounds */
    iBeaconPeriod = correctAdvertisementPeriod(iBeaconIntervalIn);
